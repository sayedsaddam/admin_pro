 			...
 			<div class="card">
 				...
 				<div class="card-body">
 					...
 					<div class="row">
 						...
 						<div class="col-md-6">
 							<p>
 								<?= date('M d, Y', strtotime($order[0]->po_date)); ?>
 							</p>
 						</div>
 					</div>
 					<?php else : ?>
 					...
 					<table class="table table-sm">
 						...
 						<tr class="<?php if(min($lowest_price) == $item->price){ echo 'table-success text-dark'; } ?>">
 							...
 							<?php } ?> <?php  
              if(!$this->session->userdata('user_role') == 'admin' && $item->price >= 100000){ ?> 
 							<td>
 							...
 							<?php } else{ ?>
 							<td>
 								<a data-id="<?= $item->qut_id.'/'.$item->purchase_id; ?>" class="return_item"><span
 										class="badge badge-success"><i class="fa fa-check"></i></span></a>
 							</td>
 							<?php } ?>
 							<!--/first if end for user role --> 
 							</strong>
 							</span>
 						</tr>
 						<?php endforeach;  ?>
              ...
              ?> <br>
 					<!-- Button -->
					 <!-- <?php $quotations = $this->admin_model->count_qutation($item->po_id); ?>  -->
 					<!-- <?php if($count < 3) { ?>
 					<a data-toggle="modal" data-target="#add_qutation" class="btn btn-outline-info d-print-none"><i
 							class="fa fa-plus"></i> Add New</a>
 					<?php } else { ?>
 					<h3 class="text-danger d-print-none">Quotation range is completed</h3>
 					<a data-toggle="modal" data-target="#add_qutation" class="btn btn-outline-danger disabled d-print-none"><i
 							class="fa fa-plus"></i> Add New</a>
 		...
 		<div class="modal-content">
 			...
 			<div class="modal-body mx-3">
 				<form action="<?= base_url('Purchase/add_qutation'); ?>" method="post">
 					<input type="hidden" name="requested_by" value="<?=$ids[0]->req_by;?>">
 					<input type="hidden" name="supplier_id" value="<?=$ids[0]->sup_id;?>">
 					<input type="hidden" name="purchase_id" value="<?=$ids[0]->purchase_id;?>">
 ...
 <div class="modal fade" id="item_return" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
 	<div class="modal-dialog" role="document">
 		<div class="modal-content">
 			...
 			<div class="modal-body mx-3">
 				<form action="<?= base_url('Purchase/approved_order'); ?>" method="post" enctype="multipart/form-data">
